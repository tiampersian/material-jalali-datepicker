...
import { DatePickerComponentPage } from './date-picker.component.integration.spec.page';
import { expectedValue } from './date-picker.component.spec.builder';

describe('SUT(integration): DatePickerComponent', () => {
  let sutPage: DatePickerComponentPage;

  CoreBrowserKendoModuleTestBedFactory();

  beforeEach(() => {
    sutPage = new DatePickerComponentPage();
  });
  ...
  it(`should populate element binding with proper value`, async () => {
    ...
    // assert
    expect(sutPage.kendoDatePicker).toHaveBeenConfiguredWith({
  ...
  it(`should populate element binding with proper value`, async () => {
    ...
    // assert
    // const tem = sutPage.datePickerDe.injector.get(NgControl)
